...
def find_closest(location, centroids):
    ...
    [2.0, 3.0]
    """
    min = distance(location, centroids[0])
    clotest = centroids[0]
    for center in centroids:
        dist = distance(location, center)
        if dist < min :
            min = dist
            clotest = center
    return clotest
...
def find_centroid(cluster):
    ...
    centor_latitude = [restaurant_location(restaurant)[0] for restaurant in cluster]
    centor_longitude = [restaurant_location(restaurant)[1] for restaurant in cluster]
    return [mean(centor_latitude),mean(centor_longitude)]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    Syy = sum([(y - mean_ys) ** 2 for y in ys])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    predictor = [list(find_predictor(user, reviewed, fn)) for fn in feature_fns]

    predictor = max(predictor, key = lambda x : x[1])
    return predictor[0]
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        if restaurant in reviewed:
            ratting[restaurantName] = user_rating(user, restaurantName)
        else :
            ratting[restaurantName] = predictor(restaurant)
